...
app.userdb.editBehaviour = function(bID, bName, bDescription, bTimeout) {
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("UPDATE behaviours SET behaviour_name=?, behaviour_description=?, behaviour_timeout=? WHERE behaviour_ID=?", [bName, bDescription, bTimeout, bID], app.userdb.onSuccess, app.userdb.onError);
	});
}
