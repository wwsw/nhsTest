...
//Add User
app.userdb.addUser = function(username, password, successFunction) {
	
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("INSERT INTO users(username, password) VALUES(?,?)", 
		[username, password],
		function(){app.userdb.onSuccess(successFunction)},
		app.userdb.onError);
	});
}

//Add Patient
app.userdb.addPatient = function(pID, pfname, plname, behavioursArray, successFunction) {
...
//Add Behaviour
app.userdb.addBehaviour = function(bname, bdescription, bTimeout, successFunction) {
...
//Add Log
app.userdb.addLog = function(bID, pID, uID, successFunction) {
...
//Edit password
app.userdb.editPassword = function(uID, password, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
...
//Edit Patient
app.userdb.editPatientName = function(pID, pfname, plname, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("UPDATE patients SET patient_fname=?, patient_lname=? WHERE patient_ID=?", [pfname, plname, pID], function(){app.userdb.onSuccess(successFunction)}, app.userdb.onError);
	});
}

//Assign Behaviours to patient

app.userdb.assignBehaviours = function(pID, behavioursArray, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("UPDATE patients SET assigned_behaviours=? WHERE patient_ID=?", [behavioursArray, pID], function(){app.userdb.onSuccess(successFunction)}, app.userdb.onError);
	});
}

//Edit Behaviour
app.userdb.editBehaviour = function(bID, bName, bDescription, bTimeout, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("UPDATE behaviours SET behaviour_name=?, behaviour_description=?, behaviour_timeout=? WHERE behaviour_ID=?", [bName, bDescription, bTimeout, bID], function(){app.userdb.onSuccess(successFunction)}, app.userdb.onError);
	});
}

//Delete Patient
app.userdb.deletePatient = function(pID, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("DELETE FROM patients WHERE patient_ID=?", [pID], function(){app.userdb.onSuccess(successFunction)}, app.userdb.onError);
	});
}

//Delete Behaviour
app.userdb.deleteBehaviour = function(bID, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
		t.executeSql("DELETE FROM behaviours WHERE behaviour_ID=?", [bID], function(){app.userdb.onSuccess(successFunction)}, app.userdb.onError);
	});
}

//Delete Patient Log
app.userdb.deletePatientLog = function(pID, successFunction) {
	var db = app.userdb.db;
	db.transaction(function(t){
...
app.userdb.listAllPatients = function() {
	...
	db.transaction(function(t) {
		...
		t.executeSql("SELECT * FROM patients", [], function (t, r) {
			...
			// save patients object to localStorage
...
app.userdb.listAllBehaviours = function() {
	...
	db.transaction(function(t) {
		...
		t.executeSql("SELECT * FROM behaviours", [], function (t, r) {
			...
			// save behaviours object to localStorage
...
app.userdb.getLog = function(pID, bID, dateInterval) {
	...
	db.transaction(function(t) {
		t.executeSql("SELECT behaviour_ID, COUNT(*) AS behaviourFrequency, strftime(?, log_timestamp) AS dategroup FROM log WHERE patient_ID=? AND behaviour_ID=? GROUP BY behaviour_ID, dategroup", [dateInterval, pID, bID], function (t, r) {
			...
			// save log object to localStorage
